...
export class PostRepository {
    ...
    return from(
      ...
      switchMap((users) => {
        ...
        return from(users).pipe(
          mergeMap((user) =>
            this.httpService
              .post(`${this.pushServer}/api/push`, {
                deviceId: user.uuid,
              })
              .pipe(
                catchError((error) => {
                  console.error(
                    `Failed to send notificaiton to ${user.uuid}`,
                    error
                  );
                  return of(null);
                })
  ...
  async create(
    { title, content, categoryId }: CreatePostDto,
    authorId: string
  ...
  async userPostList(userId: string, skipNumber: number) {
    try {
